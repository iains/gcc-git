/* C++ contracts.
   ...
   C++20 contracts.
   Rewritten for C++26 contracts by Nina Ranns (dinka.ranns@googlemail.com)
   and Iain Sandoe (iain@sandoe.co.uk)

   Acknowledgements:
   Ville Voutilainen (ville.voutilainen@gmail.com) for many useful discussions
   and ideas to evaluate.
...
init_builtin_contract_violation_type ()
{
  ...
  tree fields = get_p9600_contract_violation_fields ();
...
void update_contract_arguments(tree srcdecl, tree destdecl)
{
  ...
  if (!src_contracts)
	...
	set_fn_contract_specifiers (srcdecl,
