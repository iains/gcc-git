...
/* Should we break pre/post contracts into separate functions?  */

static bool
outline_contracts_p ()
{
  return !flag_contract_checks_inlined;
}
...
build_contract_check (tree contract)
{
  ...
  if (flag_contract_checks_inlined && POSTCONDITION_P (contract))
    {
...
should_contract_wrap_call (bool do_pre, bool do_post, bool is_virt)
{
  ...
  return ((flag_contract_client_check > 0) && do_pre);
}
