...
update_fn_contract_specifiers (tree decl, tree list)
{
  ...
  if (contract_decl *p = hash_map_safe_get (contract_decl_map, decl))
	...
	{
	  p->error_seen = true;
	  p->first_decl_specifiers = error_mark_node;
	}
      else
	  ...
	  if (diagnose
	      ...
	      && !TREE_READONLY (t2))
	    {
	      ...
	      error_at (DECL_SOURCE_LOCATION (t2),
			...
			"const", t2);
	      inform (DECL_SOURCE_LOCATION (t1),
...
void
check_parsed_contract_specifiers (tree decl, tree contracts,
				 unparsed_contracts_entry *e)
{
  if (!contracts)
    {
